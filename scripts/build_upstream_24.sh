...
validate_regex() {
  ...
  [[ "$value" =~ $regex ]] || { echo "Invalid $name: $value" >&2; exit 2; }
}
...
# Node 24.19.0's zlib GYP file compiles cpu_features.c when ARM NEON is enabled,
...
{
  cd "$NODE_DIR"
  ./configure --dest-cpu=arm64 --dest-os=android --shared --cross-compiling --with-intl=small-icu --openssl-no-asm
