...
if [[ "$MAKE_STATUS" -eq 0 ]]; then
  for candidate in "$SOURCE_DIR/out/Release/lib.target/libnode.so" "$SOURCE_DIR/out/Release/obj.target/libnode.so" "$SOURCE_DIR/out/Release/libnode.so"; do
    if [[ -s "$candidate" ]]; then LIBNODE="$candidate"; break; fi
  done
fi
if [[ -z "$LIBNODE" ]]; then
  log "Source build failed; use official nodejs-mobile v18.20.4 Android release"
...
if name not in assets: raise SystemExit('release asset not found: '+name)
a=assets[name]
digest=a.get('digest','')
if not digest.startswith('sha256:'): raise SystemExit('GitHub release asset has no SHA-256 digest')
